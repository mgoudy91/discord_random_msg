...
client.on("message", (message) => {
    ...
    if (message.content.includes("!random_message") ||
...
const downloadChannelHistory = (channel) => __awaiter(void 0, void 0, void 0, function* () {
    console.log("Indexing channel: " + channel.name);
    let messageArray = yield fetchAll(channel);
    // Write to file
...
const downloadHistory = (guild) => __awaiter(void 0, void 0, void 0, function* () {
...
const getChannels = (guild, channelName = "") => {
    const channels = guild.channels.cache.filter((channel) => channel.type === "text" && channel.name.includes(channelName));
    // filter out any non TextChannels
    return channels;
};
const getRandomMessage = (sourceChannel, channelToPost) => __awaiter(void 0, void 0, void 0, function* () {
    ...
    if (fs.existsSync(fileName)) {
        ...
        let messages = yield downloadChannelHistory(sourceChannel);
...
const fetchAll = (channel) => __awaiter(void 0, void 0, void 0, function* () {
    let size = 100;
    let messageArray = [];
    let lastID;
...
const sendHelp = (channel) => {
    const exampleEmbed = new discord_js_1.MessageEmbed()
        .setColor("#0099ff")
        .setTitle("HistoryBot")
        .setAuthor("History Bot", "https://media.discordapp.net/attachments/358274019482664961/607715919090810987/obamer_sphere.gif", "https://github.com/mgoudy91/discord_random_msg")
        .setDescription("Send a random message from your server's history")
        .addFields({ name: "Basic Usage", value: "`!rm` or `!random_message`" }, { name: "Specify channel", value: "`!rm c:channel_name`", inline: true }, { name: "Get help", value: "`!rm help`", inline: true })
        .setTimestamp()
...
function generateEmbed(message, channel) {
    ...
    const embed = new discord_js_1.MessageEmbed()
        ...
        .setURL(message.url)
        .setAuthor(author.username, message.author.avatarURL({ format: "png", dynamic: true }) || undefined)
        .setDescription(message.cleanContent)
        .setTimestamp(message.createdAt)
        .setFooter("Sent on", "https://media.tenor.com/YNwyN6nZ1jcAAAAi/barack-obama-sphere.gif");
    //Assume attachments have a higher priority
    let img;
    if (message.embeds && message.embeds.length > 0) {
        try {
            img = message.embeds.find((embed) => embed.type === "image");
        }
        catch (e) {
            console.log(e);
        }
    ...
    if (message.attachments && message.attachments.size > 0) {
        img = message.attachments.find((attach) => attach.url.match(/\.(jpg|jpeg|gif|png|tiff|bmp)$/) != null);
    }
    if (img && img.url) {
